...
def firstStartApache(name):
        a=cmd.getoutput("sshpass -p Cascaders1@3 ssh -o StrictHostKeyChecking=no root@127.0.0.1 docker run -p 8080:80 -dt --network=mynet0 --name=%s ggandhi27/apache_with_php"%name)
        print(a)
        return a
...
if __name__=="__main__":
        ...
        try:
                db,cursor=connectdb()
                if db:
                        chk=insertdb(cursor,name,uname,d1["ID"],d1["IP"],d1["Stime"],d1["Etime"],1,0,1,0,"0")
                        if chk==1:
